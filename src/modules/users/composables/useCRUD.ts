...
import { CreateUserDto, UsersService, UpdateUserDto } from "@/api";
...
export function useCRUD(fetchFunction: Function) {
  ...
  const createItem = async (itemData: CreateUserDto) => {
    try {
      isPending.value = true;
      const response = await UsersService.usersControllerCreate(
        itemData
      );
  ...
  const updateItem = async (id: string, itemData: UpdateUserDto) => {
    try {
      isPending.value = true;
  ...
  const deleteMultipleItems = async (ids) => {
    ...
    try {
      ...
      toast.success(
        `${ids.length} Utilizador(s) excluído(s) com sucesso!`
      );

      return true;
    } catch (individualError) {
      console.error("Error deleting users individually:", individualError);
      toast.error(
        `Erro ao excluir Utilizadors: ${individualError.message}`
      );
