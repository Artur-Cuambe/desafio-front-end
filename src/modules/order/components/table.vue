...
const handleDeleteSelected = () => {
  ...
  const confirmDelete = confirm(
    `Deseja realmente excluir ${selectedEntity.value.length} item(ns) selecionado(s)?`,
  );
...
const deleteRow = (row) => {
  const confirmDelete = confirm(`Deseja realmente excluir "${row.ref}"?`);
  if (confirmDelete) {
    emit("delete", [row]);
  }
};
